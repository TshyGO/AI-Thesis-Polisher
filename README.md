...
输出新增 `ChapterMemory.json`，包含出处、保护状态及预算覆盖信息。Prompt 高级设置可填写明确保护术语。记忆预算不足会明确标记；无法通过来源校验则报错，不退回自由文本或冒充无需修改。

出处验证不等于科学事实或语义已经验证。范围、缓存行为和真实 API 对比见 [章节记忆验证记录](benchmarks/MEMORY_VALIDATION.md)。
