...
固定合成语料上，打包到 6 段：调用数 21→11、编辑调用 12→2、token 25.4k→19.9k、正文重复倍率 3.31→0.99，15 次运行未出现漏检或误改。该语料每章正好 6 段，真实长章节的收益会小于此。改动该值会使已有建议缓存失效。测量与边界见 [打包实测记录](benchmarks/PACKING.md)。
