...
from engine.stage_models import resolve_configs, build_stage_clients, StageClient, persistable_overrides
...
class StageModelTests(unittest.TestCase):
    ...
    def test_invalid_endpoint_and_parameter_ranges(self):
        for url in ('https://user:pass@example.invalid/v1', 'https://example.invalid/v1?key=x', 'file:///x'):
            with self.assertRaises(ValueError):
